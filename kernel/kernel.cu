...
__device__ void output_layer_multiply(double conv_output[10][20][20], double weights[10][4000]) {
    weights[blockIdx.x*400 + threadIdx.x*20 + threadIdx.y] = conv_output[layer_idx][section_x][section_y] * weights[blockIdx.x*400 + threadIdx.x*20 + threadIdx.y];
}
...
__device__ void output_layer_add_3(double output_layer_temp_2[10], double *output) {
    ...
    if (blockIdx.x == 0 && section_x == 0 && section_y == 0) {
        double sum = 0;
        for (int i = 0; i < 10; i++) {
...
extern "C" __global__ void output_layer_for_single_output(double relu_output[10][20][20], double weights[4000], double *output) {
    output_layer_multiply(relu_output, weights);    // The output of the multiplication step is held in the weights matrix to save space, i.e. weights is modified in place
