...
impl CudaContext {
    ...
    pub fn compute(&mut self, input: &InputMatrix) -> Result<OutputVec, Box<dyn Error>> {
        ...
        unsafe {
            ...
            let mut output_layer_temp_1 = DeviceBuffer::from_slice(&[0.0f64; 4000])?;
            let mut output_layer_temp_2 = DeviceBuffer::from_slice(&[0.0f64; 200])?;
            let mut output_layer_temp_3 = DeviceBuffer::from_slice(&[0.0f64; 10])?;
            for output_idx in 0..10 {
                let result = launch!(module.output_layer_multiplication_for_single_output<<<num_blocks, threads_per_block.clone(), 0, stream>>>(
                    ...
                    output_idx,
                    output_layer_temp_1.as_device_ptr()
                ));
                result?;
                ...
                let result = launch!(module.output_layer_add_1<<<8, 25, 0, stream>>>(output_layer_temp_1.as_device_ptr(), output_layer_temp_2.as_device_ptr()));
                result?;
                let result = launch!(module.output_layer_add_2<<<1, 10, 0, stream>>>(output_layer_temp_2.as_device_ptr(), output_layer_temp_3.as_device_ptr()));
                result?;
                let result = launch!(module.output_layer_add_3<<<1, 1, 0, stream>>>(output_layer_temp_3.as_device_ptr(), single_output_box.as_device_ptr()));
                result?;
